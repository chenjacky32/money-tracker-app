<div x-data="monthSelector" x-init="const parts = '{{ $selectedMonthYear ?? now()->format('m/Y') }}'.split('/');
if (parts.length === 2) {
    datePickerMonth = parseInt(parts[0]) - 1;
    datePickerYear = parseInt(parts[1]);
    updateSelectedValue();
}"
    class="flex items-center justify-between bg-background rounded-2xl border border-gray-200 p-4 shadow-sm mb-6 relative">
    <button @click="prevMonth()" type="button"
        class="cursor-pointer w-8 h-8 flex items-center justify-center text-primary hover:bg-gray-muted rounded-full transition">
    ...
    <div class="flex items-center gap-2">
        ...
        <button @click="datePickerOpen = !datePickerOpen" type="button"
            class="cursor-pointer w-10 h-10 flex items-center justify-center text-primary bg-background border border-gray-200 rounded-xl shadow-sm hover:bg-gray-muted transition">
    ...
    <div x-show="datePickerOpen" @click.away="datePickerOpen = false" x-transition x-cloak
        ...
        <div class="flex justify-between items-center mb-4 pb-4 border-b border-gray-100">
            <button @click="datePickerYear--; updateSelectedValue()" type="button"
                class="p-1 hover:bg-gray-muted rounded-full transition text-gray-light">
                <x-ui.icon name="chevron-left" class="w-5 h-5" />
            </button>
                ...
                <button @click="selectMonth(index)" type="button"
                    :class="{
                        'bg-primary text-white': datePickerMonth ===
                            index,
                        'text-gray-darker hover:bg-gray-muted': datePickerMonth !== index
                    }"
